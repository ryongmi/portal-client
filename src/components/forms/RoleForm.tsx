...
import { mockServices, mockPermissions, mockRolePermissions } from '@/data/mockData'
...
export default function RoleForm({
  ...
  const filteredPermissions = mockPermissions.filter(permission => {
    ...
    // 서비스 필터링 제거 - 모든 서비스의 권한을 볼 수 있도록 함
    return matchesSearch
  })

  const groupedPermissions = filteredPermissions.reduce((acc, permission) => {
    const serviceName = mockServices.find(s => s.id === permission.serviceId)?.displayName || '알 수 없음'
      ...
      <div className="relative -mx-6 -mt-6">
            ...
            <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-100">
                ...
                <textarea
                  ...
                  className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all resize-none ${
        ...
        <div className="px-6 py-4 bg-gray-50 rounded-b-lg border-t border-gray-100">
          <div className="flex justify-end space-x-3">
            ...
            {activeTab === 'permissions' && (
              <Button 
                onClick={() => role && onPermissionUpdate && onPermissionUpdate(role.id, selectedPermissions)}
                className="bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white transition-all hover:scale-105"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
